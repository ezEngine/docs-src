...
If a volume additionally uses an [image](../../misc/imagedata-asset.md) (such as the [volume image component](procgen-volume-image-component.md)) to provide detailed data, the `ImageVolumeMode` is used to determine either which channel (red, green, blue or alpha) should be considered, or whether a specific color represents whether the volume should be applied. Choosing a channel means that you can only control four different things, but you have smooth values ([0; 1] range) to work with. So a channel could control *how strongly* to tint a color or how think a density should be.
