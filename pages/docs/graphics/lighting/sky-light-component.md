...
* `SpecularIntensity`: Controls the brightness of specular reflections from the skylight. Use this to reduce overly bright reflections from the environment. Note that in most cases, proper [reflection probe](reflection-probe-components.md) placement is the preferred approach to control reflections, but this property provides a quick way to adjust skylight reflection brightness when needed.
